...
export const marketMechanicsLessons: Lesson[] = [
  createLesson("order-types", 1, "Market, Limit, and Stop Orders", "order-types", "20 min", ["When to use each order type", "Pros and cons"], [
    { type: "text", heading: "Market Orders", content: "A market order tells the broker to execute immediately at the best available price. You get filled quickly, but the price can slip in fast or thin markets—you might get a worse fill than the last traded price. Use when you need to get in or out now (e.g. closing before the bell) and liquidity is good. For stop losses, a stop-market order becomes a market order when price hits your level; you're guaranteed an exit but may get slippage." },
    { type: "text", heading: "Limit Orders", content: "A limit order executes only at your specified price or better. You control the price: for a buy, you won't pay more than your limit; for a sell, you won't receive less. The trade-off: the order may not fill if price never reaches your level. Use for patient entries (e.g. 'buy when it pulls back to 50') or when you want to avoid paying the spread. Stop-limit orders combine both: when price hits your stop, a limit order is placed—more control but risk of no fill in a fast move." },
    { type: "text", heading: "Trailing Stops and Bracket Orders", content: "A trailing stop moves with price: e.g. keep the stop 2× ATR below the current price so you lock in profit as the trade runs. Bracket orders submit entry + stop loss + take profit together (OCO-style), so you don't forget to set the stop. Time-in-force: day order expires at end of day; GTC (good till canceled) stays until filled or you cancel." },
